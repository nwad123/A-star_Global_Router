...
bool CompareByBox::operator()(const IdType a, const IdType b) const
{
    ...
    CostType aWidth = abs(static_cast<CostType>(aOne.x - aTwo.x));
    CostType aHeight = abs(static_cast<CostType>(aOne.y - aTwo.y));
    pair<CostType, CostType> costA(aWidth + aHeight, min(aWidth, aHeight) / max(aWidth, aHeight));
    // net B's measurements
    CostType bWidth = abs(static_cast<CostType>(bOne.x - bTwo.x));
    CostType bHeight = abs(static_cast<CostType>(bOne.y - bTwo.y));
...
void SimpleGR::routeFlatNets(bool allowOverflow, const EdgeCost &func)
{
    ...
    SimpleProgRpt report(static_cast<uint32_t>(netIdVec.size()));
